...
class ModelState:
    ...
    def _compute_matrices(self, x:jnp.ndarray)->None:
        ...
        A[self.u_dim:2*self.u_dim, self.u_dim:2*self.u_dim] = -jnp.dot(jnp.linalg.inv(M) , c)
        A[0:self.u_dim, self.u_dim:2*self.u_dim] = jnp.eye(self.u_dim)
        A[self.u_dim:2*self.u_dim, 0:self.u_dim] = -jnp.dot(jnp.linalg.inv(M) , K)
        A[0:self.u_dim, 0:self.u_dim] = jnp.zeros((self.u_dim, self.u_dim))
        B[self.u_dim:2*self.u_dim,0:self.u_dim] =  jnp.linalg.inv(M)
        C[0:self.u_dim, 0:self.u_dim] = jnp.eye(self.u_dim)
